...
module.exports = function (currentState) {
    return co(function *() {
        ...
        let arrayOfPackageInfo = yield allDependenciesIncludingMissing
            .map(moduleName => createPackageSummary(moduleName, currentState))
            .filter(Boolean);

        if (currentState.get('majorOnly')) {
            arrayOfPackageInfo = arrayOfPackageInfo.filter(item => item.bump === 'major');
        } else if (currentState.get('notMajor')) {
            arrayOfPackageInfo = arrayOfPackageInfo.filter(item => item.bump !== 'major');
        }
